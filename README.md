...
[[https://github.com/artfin/PES-Fitting-MSA/blob/master/models/rigid/best-model/silu-svc-comparison.png|alt=octocat]]

Next, we trained a model on the dataset of intermolecular energies obtained for vibrationally excited moieties -- up to 3,000 cm-1 for CH4 and up to 1,000 for N2 (see ```models/nonrigid/```). The figure demonstrates the high quality of constructed model that can be further utilized in spectroscopic and thermophysical applications.
