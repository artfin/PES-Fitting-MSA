...
def parse_log(log_path):
    ...
    Handles two formats:
      ...
      - Force phase:  multi-line block with WMSE, MAE, RMSE for energy and force.
    """
    rows = []
    lr_map = {}
    ...
    force_epoch_re = re.compile(
        r"Epoch:\s+(\d+);\s+\(energy\)\s+WMSE train:\s+([\d.]+);\s+\(energy\)\s+WMSE val:\s+([\d.]+)"
    )
    rmse_re = re.compile(
        r"\(energy\)\s+RMSE\s+(train|val):\s+([\d.]+)\s+cm-1;\s+\(force\)\s+RMSE\s+(train|val):\s+([\d.]+)\s+cm-1/bohr"
    )
    mae_re = re.compile(
        r"\(energy\)\s+MAE\s+(train|val):\s+([\d.]+)\s+cm-1;\s+\(force\)\s+MAE\s+(train|val):\s+([\d.]+)\s+cm-1/bohr"
    )
    ...
    with open(log_path) as f:
        for line in f:
            ...
            # Force-phase epoch header
            m = force_epoch_re.search(line)
            if m:
            ...
            # RMSE lines (appear after force epoch header)
            m = rmse_re.search(line)
            if m and current_row:
                which = m.group(1)  # train or val
                current_row[f"e_rmse_{which}"] = float(m.group(2))
                current_row[f"f_rmse_{which}"] = float(m.group(4))
                if "e_rmse_val" in current_row and "f_rmse_val" in current_row:
            ...
            if m and current_row:
                ...
                current_row[f"f_mae_{which}"] = float(m.group(4))
...
def _force_start_line(ax, df_log):
    """Draw a vertical line at the first epoch with force metrics."""
    if "f_rmse_val" in df_log.columns:
        force_epochs = df_log.dropna(subset=["f_rmse_val"])
        if not force_epochs.empty:
            ep0 = force_epochs["epoch"].iloc[0]
            ax.axvline(ep0, color="grey", ls="--", lw=0.8, alpha=0.6)


def plot_training_metrics(axes, df):
    """Row 1: energy RMSE, force RMSE, LR + WMSE val."""
    ax_e, ax_f, ax_lr = axes
    ...
    ax_e.set_yscale("log")
    _force_start_line(ax_e, df)

    # Force RMSE
    if "f_rmse_train" in df.columns:
        sub = df.dropna(subset=["f_rmse_train"])
        if not sub.empty:
            ax_f.plot(sub["epoch"], sub["f_rmse_train"], label="train", lw=1)
            ax_f.plot(sub["epoch"], sub["f_rmse_val"], label="val", lw=1)
            ax_f.legend(fontsize=7)
    ax_f.set_ylabel("Force RMSE (cm⁻¹/bohr)")
    ax_f.set_title("Force RMSE")
    _force_start_line(ax_f, df)
...
def plot_trust_region(axes, df, df_log):
    ...
    _force_start_line(ax_frac, df_log)
    ...
    _force_start_line(ax_churn, df_log)
    ...
    ax_evict.set_ylabel("Force RMSE (cm⁻¹/bohr)")
    ax_evict.set_title("Eviction signal")
    ax_evict.set_yscale("symlog", linthresh=1.0)
    _force_start_line(ax_evict, df_log)


def plot_force_diagnostics(axes, df, df_log):
    ...
    ax_cq.set_title("Per-config force-loss contribution")
    ax_cq.legend(fontsize=6, ncol=2)
    _force_start_line(ax_cq, df_log)
    ...
    _force_start_line(ax_phi, df_log)
...
def plot_lbfgs_diagnostics(axes, df, df_log):
    ...
    _force_start_line(ax_sy, df_log)
    ...
    _force_start_line(ax_grad, df_log)
...
def main():
    ...
    force_path = base + ".force_diagnostics.csv"
    ...
    has_force = os.path.exists(force_path)
    has_lbfgs = os.path.exists(lbfgs_path)

    df_trust = pd.read_csv(trust_path) if has_trust else None
    df_force = pd.read_csv(force_path) if has_force else None
    ...
    if df_force is not None:
        print(f"Force diagnostics: {len(df_force)} rows")
    ...
    if has_force:
        plot_force_diagnostics(all_axes[row], df_force, df_log)
        row += 1
