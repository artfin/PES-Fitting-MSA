...
subroutine potinit() bind(C, name="potinit")
    use params
    implicit none
    ...
    !read eq parameters
    do ieq = 1, 2
        read (100, *) buf30, par(ieq)
    enddo
    !read potential parameters
     do ipar=1, parmax
        read (100, *) buf30, term(ipar), force(ipar)
    enddo
end subroutine

subroutine poten_xy4(local, f) bind(c, name='poten_xy4')
    use params
    ...
    real (c_double), intent(in)  :: local(10)
    
    real (c_double) dF(289)
