...
def plot_svc(nnpip_svc, reference_svc, figname=None):
    ...
    plt.plot(reference_svc[:,0], reference_svc[:,1], color='#6CD4FF', lw=2.0, label="Symmetry-adapted angular basis")
    plt.plot(nnpip_svc[:,0], nnpip_svc[:,1], color='#FF6F61', linestyle='--', dashes=(5, 5), lw=3.0, label="NN-PIP")
...
if __name__ == "__main__":
    BASEDIR = "/home/artfin/Desktop/neural-networks/project/PES-Fitting-MSA/"

    figname = os.path.join(BASEDIR, "models", "rigid", "best-model", "SVC-comparison.png")

    nnpip_svc     = np.loadtxt(os.path.join(BASEDIR, "models", "rigid", "best-model", "nnpip-svc.txt"))
    reference_svc = np.loadtxt("symmpes-svc.txt")

    plot_svc(nnpip_svc, reference_svc, figname=figname)
    trim_png(figname)
