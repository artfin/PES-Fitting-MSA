...
sub print_module_head{
    ...
    if ($c_bind) {
        print $out "  function emsav(x, c) result(v) bind(c, name=\"c_emsav\")\n";
        print $out "    use iso_c_binding\n";
    } else {
    ...
    if ($c_bind) {
        print $out "    real (c_double), dimension(1:$nx) :: x\n";
        print $out "    real (c_double), dimension(0:$np) :: c\n";
        print $out "    real (c_double)                   :: v\n";
        print $out "    ! ::::::::::::::::::::\n";
        print $out "    real (c_double), dimension(0:$np) :: p\n";
    } else {
    ...
    if ($c_bind) {
        print $out "  subroutine bemsav(x, p) bind(c, name=\"c_bemsav\")\n";
        print $out "    use iso_c_binding\n";
    } else {
    ...
    if ($c_bind) {
        print $out "    real (c_double), dimension(1:$nx), intent(in)  :: x\n";
        print $out "    real (c_double), dimension(0:$np), intent(out) :: p\n";
        print $out "    ! ::::::::::::::::::::\n";
        print $out "    real (c_double), dimension(0:$nm) :: m\n";
    } else {
...
sub print_mono_head{
    ...
    if ($c_bind) {
        print $om "  subroutine evmono(x, m) bind(c, name=\"c_evmono\")\n";
    ...
    if ($c_bind) {
        print $om "    real (c_double), dimension(1:$nx), intent(in)  :: x\n";
        print $om "    real (c_double), dimension(0:$nm), intent(out) :: m\n";
    } else {
...
sub print_poly_head{
    ...
    if ($c_bind) {
        print $op "  subroutine evpoly(m, p) bind(c, name=\"c_evpoly\")\n";
    ...
    if ($c_bind) {
        print $op "    real (c_double), dimension(0:$nm), intent(in)  :: m\n";
        print $op "    real (c_double), dimension(0:$np), intent(out) :: p\n";
    } else {
